...
webSocketServer.on<CustomProps>("connect", (conn) => {
	console.log(
		"New connection",
		"Clients = " + webSocketServer.countTotalClients()
	);
	...
	conn.on<SetNameMessage>("message", "set:name", (message) => {
		if (typeof message.name == "string" && message.name.length > 0) {
			...
			conn
				.send("set:name success", {
					name: conn.props.name,
				})
				.catch((error) => {
					if (error == ServerConnectionErrors.notAlive) {
						console.log(
							"Failed to send name update success message because the client has already disconnected"
						);
					}
				});
		}
		...
		setTimeout(() => {
			conn
				...
				.catch((error) => {
					if (error == ServerConnectionErrors.notAlive) {
						console.log(
							"Failed to send message because the connection isn't alive"
						);
					}
				});
		}, 1000);
	...
	conn.on("message", "reboot", () => {
		...
		webSocketServer.stop().then(() => {
			webSocketServer.run().catch((err) => {
				console.log(
					"Error occurred when starting the server, exit code " + err
				);
			});
		});
	});
	...
	conn.on("disconnect", (code) => {
		console.log(
			"Connection disconnected, CODE = " + code,
			"Clients = " + webSocketServer.countTotalClients()
		);
	});
});
