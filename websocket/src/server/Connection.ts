...
export default class Connection<CustomProperties> {
	...
	public constructor(
		...
		server: Server,
		onFinish: () => void,
		onDeath: (identifier: string, stateUpdatesDone: () => void) => void
	) {
				...
				.catch((error) => {
					this.events.messageError.forEach((event) => {
						if (event.channel.length == 0) {
							event.listener(
								new Error(
									`An error ocurred while parsing the JSON data in the message, JSON error: "${error.message}"`
								)
							);
						}
					});
	...
	public send<MessageType>(
		...
		return new Promise((resolve, reject) => {
			...
			try {
				...
				if (
					!this.realWebSocketConnection.CLOSING ||
					!this.realWebSocketConnection.CLOSED
				) {
