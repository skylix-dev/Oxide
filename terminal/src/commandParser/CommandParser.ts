...
export default class CommandParser {
	...
	public constructor(settings: Settings = {}) {
		const defaultSettings = {
			defaultHelpPageRenderer: (commands, page, isInvalid) => {
				...
				if (page < 1) {
					logging.warning(
						"Page " + page + " does not exist, showing results for page 1"
					);
					page = 1;
				}
			...
			usageErrorRenderer: (name, invalid, missing) => {
				logging.info(
					`Invalid usage for command "${name}", use "help ${name}" to get a detailed list of commands and options`
				);
		...
		this.registerCommand<HelpCommandFlags>({
			...
			handler: (args, options) => {
				...
				this.settings.defaultHelpPageRenderer!(
					commands,
					options.page ?? 1,
					false
				);
	 ...
	 */
	public getCommand<FlagTypes>(
		commandName: string
	): Command<FlagTypes> | undefined {
		for (const command in this.commandRegistry) {
			if (this.commandRegistry[command].name == commandName) {
		...
		return new Promise((resolve, reject) => {
			...
			if (this.commandExists(parsedCommandData._[0] ?? "")) {
				...
				if (registryCommand?.options && registryCommand.options.length > 0) {
					...
					const observeDataType: (
						input: any
					) => "string" | "boolean" | "number" = (input: any) => {
					...
					registryCommand.options.forEach((option) => {
						if (
							option.required &&
							!commandOptions.hasOwnProperty(option.name)
						) {
					...
					if (missingFlags.length != 0) {
						this.settings.usageErrorRenderer!(
							parsedCommandData._[0],
							invalidFlags,
							missingFlags
						);
					...
					if (missingFlags.length == 0) {
						for (const inputFlag in commandOptions) {
							if (
								!(
									registryCommand.options.filter((opt) => opt.name == inputFlag)
										.length > 0
								)
							) {
						...
						if (invalidFlags.length != 0) {
							...
							registryCommand.options.forEach((option) => {
								if (option.type && commandOptions.hasOwnProperty(option.name)) {
									const optionType = observeDataType(
									...
									switch (option.type) {
										case "boolean":
											if (optionType != "boolean") {
												if (
													commandOptions[option.name].toLowerCase() == "true" ||
													commandOptions[option.name].toLowerCase() == "false"
												) {
													commandOptions[option.name] =
														commandOptions[option.name].toLowerCase() == "true"
															? true
							...
							if (typeErrorFlags.length == 0) {
								registryCommand.handler(
									parsedCommandData._.splice(1),
									commandOptions
								);
								resolve();
								return;
			...
			if (
				parsedCommandData._.length != 0 &&
				!this.commandExists(parsedCommandData._[0])
			) {
	 ...
	 */
	public registerCommand<FlagTypes>(
		command: Command<FlagTypes>
	): Promise<string> {
		return new Promise((resolve, reject) => {
